...
const EditCrediting = ({
  ...
  const [options, setOptions] = useState([
    { key: "1", value: "افزایش" },
    { key: "2", value: "کاهش" },
    //   { key: "3", value: "فعال کردن" },
    //   { key: "4", value: "غیر فعال کردن" },
  ]);
  ...
  }, [updateType, amount, reset]);

    const mutation = useMutation({
      mutationFn: LedgerAccountUpdateCredit,
      onMutate: () => {
        setLoad3(true);
      },
      onSuccess: (result) => {
  ...
  return (
    ...
    >
      ...
      >
        <p className="w-full">نوع بروزرسانی اعتبار :</p>
        <Controller
          name="updateType"
          control={control}
          rules={{
            required: {
              value: true,
              message: " انتخاب نوع بروزرسانی اعتبار الزامی است",
            },
          }}
          render={({ field }) => (
            <InputSelect
        ...
        />
        {errors.updateType && <p style={{ color: "red" }}>{errors.updateType.message}</p>}
      </div>
