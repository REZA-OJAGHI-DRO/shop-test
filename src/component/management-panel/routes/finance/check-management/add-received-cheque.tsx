...
const AddReceivedCheque: FC<Props> = ({ prevStep, chequeData }) => {
  ...
  const addReceivedChequeMutation = useMutation({
    mutationKey: ["cheque-add-received"],
    mutationFn: chequeService.addReceivedCheque,
    onSuccess: (data) => {
      if (data.isSuccess) {
        toast({
          variant: "success",
  ...
  function addCheque(imageId?: string) {
    ...
    let sender = RelationshipMemberTypeEnum.Supplier;
    if (cheque.senderUserId) {
      const type = relationshipsList?.find(
        (item) => item.value === cheque.senderUserId
      )?.userType;
      if (type !== undefined) sender = type;
    }
    addReceivedChequeMutation.mutate({
      ...cheque,
      ...
      chequeImage: imageId || "",
      senderUserId: cheque.senderUserId || userData?.userId || "",
      senderType: sender,
    });
  }
        ...
        <form className="h-full" onSubmit={form.handleSubmit(onSubmit)}>
          <div className="p-6 bg-gray-50/50 text-medium text-gray-500 dark:text-gray-400 dark:bg-gray-800 rounded-lg w-full">
            ...
            <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-5 text-gray-very-dark">
              ...
              <FormField
                control={form.control}
                name="cheque.senderUserId"
                render={({ field }) => (
                  <FormItem className="w-full">
                    <FormLabel> دریافت شده از :</FormLabel>
                    <FormControl>
                      <Select
                        onValueChange={field.onChange}
                        value={field.value}
                          ...
                          <SelectValue
                            placeholder={
                              relationships.isLoading
                                ? "در حال دریافت اطلاعات ..."
                                : "همکاری که چک را از او دریافت کردید را انتخاب کنید"
                            }
                          />
                        </SelectTrigger>
            ...
            <div className="flex md:flex-row flex-col justify-center gap-5 mt-10">
              ...
              <Button
                ...
                disabled={
                  ...
                  addReceivedChequeMutation.isPending
                }
              >
                {uploadImageMutation.isPending ||
                addReceivedChequeMutation.isPending ? (
...
export default AddReceivedCheque;
