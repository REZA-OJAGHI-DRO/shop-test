...
function TextFull({
  ...
  const handleChange = (e) => {
    ...
    // حذف اسپیس اول در صورت وجود
    if (inputValue.startsWith(" ")) {
      inputValue = inputValue.trimStart(); // فقط اسپیس‌های اول را حذف می‌کند
    }

    // ست کردن مقدار نهایی در state
    setData(inputValue);
  };
  ...
  return (
    <>
      <div className={`${width} flex flex-wrap gap-2 lg:gap-3`}>
        ...
        >
          <input
            onChange={handleChange} // استفاده از handleChange برای مدیریت تغییرات
            value={data === null ? "" : data}
            type="text"
            className={`${styleInput} bg-transparent w-full text-right px-4 focus:border-transparent focus:outline-none placeholder:text-center text-gray-very-dark`}
