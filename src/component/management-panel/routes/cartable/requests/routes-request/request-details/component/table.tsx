//@ts-nocheck
import { ShoppingCartGetAllResponseType , ShoppingCartGetAllResponseItemType } from "@/types/shopping-cart-types";
...
const Table: FC<Props> = ({ order }) => {
  ...
  const handleSubmitEdit = async () => { 
    ...
    try {
      const response: ApiResponse<ShoppingCartEditRequestType> = await shoppingCartService.getEditCart(requestPayload);
...
// },[dataDiscount])
            ...
            <table className="w-full table-auto border-collapse  overflow-hidden">
              <thead className="h-[50px]">
                <tr className="border-b border-zinc-500 h-[50px] *:text-zinc-600">
                  <th className="w-fit text-center  border-e border-zinc-300">ردیف</th>
                  <th className="w-fit text-center  border-e border-zinc-300">کد کالا</th>
                  <th className="w-fit text-center  border-e border-zinc-300">نام کالا</th>
                  <th className="w-fit text-center  border-e border-zinc-300">تعداد کارتن</th>
                  <th className="w-fit text-center  border-e border-zinc-300">تعداد واحد در کارتن</th>
                  <th className="w-fit text-center  border-e border-zinc-300">تعداد واحد در زیر کارتن</th>
                  <th className="w-fit text-center  border-e border-zinc-300">تعداد نهایی</th>
                  <th className="w-fit text-center  border-e border-zinc-300">واحد</th>
                  <th className="w-fit text-center  border-e border-zinc-300">قیمت اصلی</th>
                  <th className="w-fit text-center  border-e border-zinc-300">تخفیف</th>
              ...
              <tbody>
                {data &&
                  data?.map((val, i) => (
                    ...
                    >
                      <td className="w-fit text-center border-e border-zinc-300">{i + 1}</td>
                      <td className="w-fit text-center border-e border-zinc-300">{val.goodCode}</td>
                      <td className="w-fit text-center border-e border-zinc-300">{val.goodName}</td>
                      <td className="w-fit text-center border-e border-zinc-300">
                        ...
                        <td className="w-fit text-center border-e border-zinc-300">{val.countInBox * val.packageCount}</td>
                      <td className="w-fit text-center border-e border-zinc-300">
                        <input
                          dir="ltr"
                          type="number"
                        ...
                        <td className="w-fit text-center border-e border-zinc-300">{val.countInBox * val.packageCount + val.subPackageCount}</td>
                      <td className="w-fit text-center border-e border-zinc-300">{val.unit}</td>
                      <td className="w-fit text-center border-e border-zinc-300">
                      ...
                      <td className="w-fit text-center">
                        {(
                          ((val.packageCount * val.countInBox + val.subPackageCount) * val.priceAmount) *
                ...
                <tr className="h-[50px] *:text-zinc-600">
                  ...
                  <th className="w-fit text-center">
                    ...
                    <th className="w-fit text-center">
                    {data?.reduce(
                      (total, item) => total + Number(item.countInBox * item.packageCount + item.subPackageCount),
                      0
                    )}
                    </th>
                  <th className="w-fit text-center"></th>
                  ...
                  <th className="w-fit text-center">
                    {data?.reduce(
                      (total, item) => total + Number(((item.packageCount * item.countInBox + item.subPackageCount) * item.priceAmount) *
   ...
   <div className="w-full text-[1.1rem] flex flex-wrap">
      {dataDiscount.map((val, index) => (
        <div key={index} className="w-full flex justify-between items-center">
          <p>
            {val?.type === 2
              ? `اضافه شدن تخفیف زیر فاکتور با عنوان ${val?.title} به مقدار ${Number(val?.price).toLocaleString("fa-IR")}`
              : val?.type === 1
              ? `اضافه شدن اضافات زیر فاکتور با عنوان ${val?.title} به مقدار ${Number(val?.price).toLocaleString("fa-IR")}`
              : ''}
          </p>
          <div
        ...
        <div className="w-full py-5">
          <Description setShowDiscount={setShowDiscount} description={description} setDescription={setDescription}/>
        </div>
          ...
          <p>
  {
    (data?.reduce(
      (total, item) => 
        total + Number(
          ((item.packageCount * item.countInBox + item.subPackageCount) * item.priceAmount) *
          (1 - (item.goodDiscountPercent || 0) / 100)
        ), 
      0
    ) 
    ...
    (
      data?.reduce(
        (total, item) =>
          total + (
            ...
            item.priceAmount * 
            (item.goodDiscountPercent || 0) / 100
          ),
        0
      )
      + (dataDiscount?.filter(val => val.type === 2).reduce((sum, val) => sum + Number(val.price), 0))
    ).toLocaleString("fa-IR")
  }
</p>
      ...
      {showDiscount && (
        <Modal onClose={closeModal} title="" style={"w-[90vw] xl:w-[600px]"}>
          ...
          <FormDiscount setTitle={setTitle} title={title} price={price} setPrice={setPrice} type={type} setType={setType}  setShowDiscount={setShowDiscount} setShowDiscount2={setShowDiscount2}/>
          </div>
        </Modal>
      )}
      {showDiscount2 && (
