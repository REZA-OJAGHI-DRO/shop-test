...
const InqueryRecordsList: FC<Props> = ({}) => {
    ...
    <div>
      ...
      >
        <div className="overflow-x-auto">
          <div className="grid grid-cols-6 text-sm [&_div]:p-2 ps-6 pe-5 text-center text-gray-very-dark min-w-[700px]">
            <div>شناسه صیاد</div>
            <div>صاحب</div>
            ...
            <div className="overflow-hidden  rounded-2xl text-gray-very-dark min-w-[700px]">
              {filteredCheques && filteredCheques.length > 0 ? (
                filteredCheques.map((row) => (
                  <div
                    className={`grid grid-cols-6 items-center bg-[rgba(255,255,255,0.6)] text-sm [&_div]:p-2 ps-6 text-center pe-5 [&_div]:overflow-hidden [&_div]:text-ellipsis py-3`}
                    key={row.id}
                  >
                    <div>{row?.sayadId || ""}</div>
                    ...
                    <div>
                      {row?.seriesNo && row?.serialNo
                        ? `${row.seriesNo}/${row.serialNo}`
                        : ""}
                    </div>

                    <div>{row.enabled ? : "نا"}</div>
...
export default InqueryRecordsList;
